	...
	try{
		...
		if ($serviceName != NULL && $serviceName != ""){
			...
			if (isset($request_data["withLog"]) && $request_data["withLog"]==1){
				...
				$strSQLComp=" WHERE not exists (SELECT 'x' FROM hits h WHERE h.serviceName=s.serviceName)";
			}
			...
			if (isset($request_data["withQuotas"]) && $request_data["withQuotas"]=1){
...
function createService($serviceName = NULL, $request_data=NULL){
	...
	if ($request_data["isPublished"]=="1" ||  $request_data["isPublished"]=="0"){
		...
		$error->setHttpStatus(400);
		$error->setFunctionalCode(1);
		$error->setFunctionalLabel($error->getFunctionalLabel() . " allowed value for isPublished are 0 or 1\n");
	}
