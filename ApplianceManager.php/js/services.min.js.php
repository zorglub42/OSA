var currentService,currentServiceGroup,serviceModified,serviceNameFilterPrevVal="",serviceGroupNameFilterPrevVal="",frontEndEndPointFilterPrevVal="",backEndEndPointFilterPrevVal="";function setServiceModified(a){(serviceModified=a)?setActionButtonEnabled("saveService",!0):setActionButtonEnabled("saveService",!1);""!=$("#additionalConfiguration").val()?$("#warnAdditionalConfig").show():$("#warnAdditionalConfig").hide()}function startEditService(a){$.getJSON(a,editService).error(displayErrorV2)}
function startPopulateGroups(){$.getJSON("groups/",populateGroups).error(displayErrorV2)}function populateGroups(a){$.each(a,function(a,b){$("#groupName").append($("<option>",{value:b.groupName,text:""!=b.description?b.description:b.groupName}))});$("#groupName option[value="+currentServiceGroupName+"]").prop("selected","selected")}function updateService(a){saveOrUpdateService("PUT")}
...
function editService(a){$.get("resources/templates/serviceEdit.php",function(c){currentService=a;currentServiceGroup=a.groupUri;currentServiceGroupName=a.groupName;cbIsUserAuthenticationEnabled=cbIsPublished="checked";cbIsHitLoggingEnabled=cbIsAnonymousAllowed="";cbOnAllNodes="checked";cbUserQuota=cbIdentFwd="";1==a.isUserQuotasEnabled&&(cbUserQuota="checked");1==a.isIdentityForwardingEnabled&&(cbIdentFwd="checked");0==a.isPublished&&(cbIsPublished="");0==a.isUserAuthenticationEnabled&&(cbIsUserAuthenticationEnabled=
"");1==a.isHitLoggingEnabled&&(cbIsHitLoggingEnabled="checked");1==a.isAnonymousAllowed&&(cbIsAnonymousAllowed="checked");0==a.onAllNodes&&(cbOnAllNodes="");$("#content").html(c.replaceAll("{uri}",a.uri).replaceAll("{reqSec}",0==a.reqSec?"":a.reqSec).replaceAll("{reqDay}",0==a.reqDay?"":a.reqDay).replaceAll("{reqMonth}",0==a.reqMonth?"":a.reqMonth).replaceAll("{cbUserQuota}",cbUserQuota).replaceAll("{serviceName}",a.serviceName).replaceAll("{serviceNameInputType}","hidden").replaceAll("{cbIsPublished}",
...
function saveNewService(){saveOrUpdateService("POST")}function setNodesVisiblility(){document.getElementById("onAllNodes").checked?$("#publishedOnNodes").hide():($("#publishedOnNodes").show(),$.getJSON("services/"+encodeURIComponent($("#serviceName").val())+"/nodes/?order=nodeName",displayServiceNodes).error(displayErrorV2))}
...
function addService(){$.get("resources/templates/serviceAdd.php",function(a){currentServiceGroupName=currentServiceGroup=currentService=null;$("#content").html(a.replaceAll("{uri}","").replaceAll("{reqSec}","").replaceAll("{reqDay}","").replaceAll("{reqMonth}","").replaceAll("{cbUserQuota}","").replaceAll("{serviceName}","").replaceAll("{serviceNameInputType}","text").replaceAll("{cbIsPublished}","checked").replaceAll("{backEndEndPointValue}","").replaceAll("{frontEndEndPointValue}","").replaceAll("{cbIsUserAuthenticationEnabled}",
"").replaceAll("{loginFormUri}","").replaceAll("{cbIsAnonymousAllowed}","").replaceAll("{cbIdentFwd}","").replaceAll("{backEndUsername}","").replaceAll("{backEndPassword}","").replaceAll("{cbOnAllNodes}","checked").replaceAll("{cbIsHitLoggingEnabled}","").replaceAll("{additionalConfiguration}",""));$(function(){$("#tabs").tabs()});startPopulateGroups();setQuotasVisibility();setNodesVisiblility();setServiceModified(!1);$("#mainForm").height($("#tabs").height()+10);$("#mainForm").click(function(){$("#mainForm").height($("#tabs").height()+
10)})})}function handelServiceFilterFormKeypress(a){if(13==a.keyCode)return showServices(),!1}
function displayServiceNodes(a){$("#serviceNodesList").find("option").remove().end();$.each(a,function(a,b){port="";1==b.node.isHTTPS?(nodePrefix="https://",443!=b.node.port&&(port=":"+b.node.port)):(nodePrefix="http://",80!=b.node.port&&(port=":"+b.node.port));optionText=b.node.nodeName+" ("+nodePrefix+b.node.serverFQDN+port+")";$("#serviceNodesList").append($("<option>",{value:b.node.nodeName,text:optionText}));b.published&&$("#serviceNodesList option[value="+b.node.nodeName+"]").prop("selected",
"selected")})}
...
a[i].groupName).replaceAll("{serviceList[i].frontEndEndPoint}",a[i].frontEndEndPoint).replaceAll("{serviceList[i].backEndEndPoint}",a[i].backEndEndPoint).replaceAll("{serviceList[i].uri}",a[i].uri),table.appendChild(newRow),edit=document.getElementById("btnEdit"),del=document.getElementById("btnDelete"),a[i].serviceName.startsWith("ApplianceManagerAdmin")?del.remove():del.removeAttribute("id"),edit.removeAttribute("id");setServiceModified(!1)})}
function deleteService(a,c){confirm("<?php echo Localization::getJSString("service.delete.confirm")?> "+c+"?")&&(showWait(),$.ajax({url:a,dataType:"json",type:"DELETE",success:showServices,error:displayErrorV2}))}function resetServiceFilter(){$("#serviceNameFilter").val("");$("#serviceGroupNameFilter").val("");$("#frontEndEndPointFilter").val("");$("#backEndEndPointFilter").val("");showServices()}
function showServices(){prms="order=serviceName";prms=prms+"&serviceNameFilter="+encodeURIComponent(getFilterValue("serviceNameFilter"));prms=prms+"&groupNameFilter="+encodeURIComponent(getFilterValue("serviceGroupNameFilter"));prms=prms+"&frontEndEndPointFilter="+encodeURIComponent(getFilterValue("frontEndEndPointFilter"));prms=prms+"&backEndEndPointFilter="+encodeURIComponent(getFilterValue("backEndEndPointFilter"));$.ajax({url:"./services/",dataType:"json",type:"GET",data:prms,success:displayServiceList,
error:displayErrorV2})}$(function(){$("#listService").click(resetServiceFilter);$("#addService").click(addService)});
