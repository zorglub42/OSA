var currentUserURI,currentUser,userModified,propertiesCount,editQuota="Edit this quota",deleteQuota="Delete this quota",userNameFilterPrevVal="",firstNameFilterPrevVal="",lastNameFilterPrevVal="",emailAddressFilterPrevVal="",entityFilterPrevVal="";
function deleteUserQuotas(a,b){confirm("<?php echo Localization::getJSString("user.deleteQuota.confirm")?> "+b+"?")&&$.ajax({url:a,dataType:"json",type:"DELETE",success:startDisplayUserQuotasForCurrentUser,error:displayErrorV2})}
function setUserModified(a){(userModified=a)?($("#cancel").html("<?php echo Localization::getJSString("button.cancel")?>"),setActionButtonEnabled("saveNew",!0),setActionButtonEnabled("saveEdit",!0),setActionButtonEnabled("groupsEdit",!1),setActionButtonEnabled("quotasEdit",!1)):($("#cancel").html("<?php echo Localization::getJSString("button.back")?>"),setActionButtonEnabled("saveNew",!1),setActionButtonEnabled("saveEdit",!1),setActionButtonEnabled("groupsEdit",!0),setActionButtonEnabled("quotasEdit",
!0))}function deleteUserGroup(a,b,c){confirm("<?php echo Localization::getJSString("user.deleteGroup.confirm")?> "+b+"?")&&$.ajax({url:c+"/"+a,dataType:"json",type:"DELETE",success:startDisplayUserGroupsForCurrentUser,error:displayErrorV2})}
...
function addUser(){$.get("resources/templates/userAdd.php",function(a){currentServiceGroup=currentService=null;$("#content").html(a.replaceAll("{userNameAsLabel}","").replaceAll("{userNameInputType}","text").replaceAll("{userName}","").replaceAll("{password}","").replaceAll("{firstName}","").replaceAll("{lastName}","").replaceAll("{entity}","").replaceAll("{emailAddress}","").replaceAll("{endDate}",""));$("#userEndDate").datepicker();$("#lastTokenLogin").hide();$("#lastTokenLoginLabel").hide();propertiesCount=
0;setUserModified(!1)})}function saveNewUser(){saveOrUpdateUser("POST")}function updateUser(a){saveOrUpdateUser("PUT")}
function saveOrUpdateUser(a){if(!addUserProperty())return!1;currentUserUri="users/"+encodeURIComponent(document.getElementById("userName").value);try{d=Date.parseExact(document.getElementById("userEndDate").value,"<?php echo Localization::getJSString("date.format.parseexact")?>"),d.setHours(12),endDate=d.format("isoUtcDateTime")}catch(b){endDate=""}user={userName:$("#userName").val(),password:$("#userPass").val(),email:$("#userMail").val(),firstName:$("#firstName").val(),lastName:$("#lastName").val(),
entity:$("#entity").val(),properties:[],endDate:endDate};for(i=0;i<propertiesCount;i++)void 0!==$("#propertyName_"+i).val()&&void 0!==$("#propertyValue_"+i).val()&&(prop={name:$("#propertyName_"+i).val(),value:$("#propertyValue_"+i).val()},user.properties.push(prop));"POST"==a?(uri="users/",postData="userName="+encodeURIComponent(document.getElementById("userName").value)):uri="users/"+encodeURIComponent(document.getElementById("userName").value);$.ajax({url:uri,dataType:"json",contentType:"application/json",
type:a,data:JSON.stringify(user),success:startEditCurrentUser,error:displayErrorV2})}function startEditUser(a){currentUserUri=a;$.getJSON(a,editUser).error(displayErrorV2)}function startEditCurrentUser(){startEditUser(currentUserUri)}function startDisplayAvailableGroups(a){currentUserURI=a;$.getJSON(a+"/groups/available/",displayAvailableGroups).error(displayErrorV2)}function startDisplayUserGroupsForCurrentUser(a){startDisplayUserGroups(currentUserURI)}
function startDisplayUserGroups(a){currentUserURI=a;$.getJSON(a+"/groups/",displayUserGroups).error(displayErrorV2)}function startDisplayUserQuotas(a){currentUserURI=a;$.getJSON(a+"/quotas/",displayUserQuotas).error(displayErrorV2)}function startDisplayUserQuotasForCurrentUser(){startDisplayUserQuotas(currentUserUri)}
function displayAvailableGroups(a){0<a.length?($("#availableGroupsList").empty(),$.each(a,function(a,c){$("#availableGroupsList").append($("<option>",{value:"groups/"+c.groupName,text:c.groupName}))})):$("#addGroups").hide()}
function addUserProperty(){if(""!=$("#propertyName_new").val()&&""!=$("#propertyValue_new").val()){for(i=0;i<propertiesCount;i++)if($("#propertyName_"+i).val()==$("#propertyName_new").val())return alert("<?php echo Localization::getJSString("user.property.exists")?>: "+$("#propertyName_new").val()),!1;table=document.getElementById("data");rowPattern=document.getElementById("rowTpl");newRow=rowPattern.cloneNode(!0);newRow.removeAttribute("id");newRow.setAttribute("id","property_"+propertiesCount);
newRow.removeAttribute("style");newRow.className=newRow.className+" tabular_table_body"+propertiesCount%2;newRow.innerHTML=newRow.innerHTML.replaceAll("{i}",propertiesCount).replaceAll("{propertiesList[i].name}",$("#propertyName_new").val()).replaceAll("{propertiesList[i].value}",$("#propertyValue_new").val());table.insertBefore(newRow,document.getElementById("newProp"));$("#propertyValue_new").val("");$("#propertyName_new").val("");propertiesCount++;setUserModified(!0)}return!0}
function deleteUserProperty(a,b){confirm("<?php echo Localization::getJSString("user.property.delete.confirm")?> "+b+"?")&&(prop=document.getElementById("property_"+a),prop.parentNode.removeChild(prop),setUserModified(!0))}
function editUser(a){$.get("resources/templates/userEdit.php",function(b){null!==a.endDate?(userDate=new Date(a.endDate),dateFormated=userDate.format("<?php echo Localization::getJSString("date.format")?>")):dateFormated="";null!=a.lastTokenLogin?(loginDate=new Date(a.lastTokenLogin),lastTokenLogin=loginDate.format("<?php echo Localization::getJSString("date.format.parseexact.long")?>")):lastTokenLogin="";currentUser=a;$("#content").html(b.replaceAll("{userNameAsLabel}",
...
table.removeChild(rowPattern);b=[];var c=[],e=[],f=[],g=[];for(i=0;i<a.length;i++){if(null!==a[i].endDate){var h=new Date;h.setISO8601(a[i].endDate);dateFormated=h.format("<?php echo Localization::getJSString("date.format")?>")}else dateFormated="";addItem(b,a[i].userName);addItem(c,a[i].emailAddress,!0);addItem(e,a[i].entity,!0);addItem(f,a[i].firstName,!0);addItem(g,a[i].lastName,!0);newRow=rowPattern.cloneNode(!0);newRow.removeAttribute("id");newRow.removeAttribute("style");newRow.className=
...
function showUsers(){backUsers=showUsers;prms="order=userName";prms=prms+"&userNameFilter="+encodeURIComponent(getFilterValue("userNameFilter"));prms=prms+"&firstNameFilter="+encodeURIComponent(getFilterValue("firstNameFilter"));prms=prms+"&lastNameFilter="+encodeURIComponent(getFilterValue("lastNameFilter"));prms=prms+"&emailAddressFilter="+encodeURIComponent(getFilterValue("emailAddressFilter"));prms=prms+"&entityFilter="+encodeURIComponent(getFilterValue("entityFilter"));$.ajax({url:"./users/",
dataType:"json",type:"GET",data:prms,success:displayUserList,error:displayErrorV2})}$(function(){$("#listUser").click(resetUserFilter);$("#addUser").click(addUser)});
