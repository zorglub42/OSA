...
function editUser(a){$.get("resources/templates/userEdit.php",function(b){userDate=new Date(a.endDate);dateFormated=userDate.format("<?php echo Localization::getJSString("date.format")?>");currentUser=a;$("#content").html(b.replaceAll("{userNameAsLabel}",a.userName).replaceAll("{userNameInputType}","hidden").replaceAll("{userName}",a.userName).replaceAll("{password}",a.password).replaceAll("{firstName}",a.firstName).replaceAll("{lastName}",a.lastName).replaceAll("{entity}",a.entity).replaceAll("{emailAddress}",
a.emailAddress).replaceAll("{endDate}",dateFormated).replaceAll("{extra}",null==a.extra?"":a.extra).replaceAll("{uri}",a.uri));$("#userEndDate").datepicker();setUserModified(!1)})}
