var currentUserURI,currentUser,userModified,editQuota="Edit this quota",deleteQuota="Delete this quota",userNameFilterPrevVal="",firstNameFilterPrevVal="",lastNameFilterPrevVal="",emailAddressFilterPrevVal="",entityFilterPrevVal="";function deleteUserQuotas(a,b){confirm("<?php echo Localization::getJSString("user.deleteQuota.confirm")?> "+b+"?")&&$.ajax({url:a,dataType:"json",type:"DELETE",success:startDisplayUserQuotasForCurrentUser,error:displayErrorV2})}
...
entity="entity="+encodeURIComponent(document.getElementById("entity").value);extra="extra="+encodeURIComponent(document.getElementById("extra").value);try{d=Date.parseExact(document.getElementById("userEndDate").value,"<?php echo Localization::getJSString("date.format.parseexact")?>"),d.setHours(12),endDate="endDate="+encodeURIComponent(d.format("isoUtcDateTime"))}catch(b){endDate="endDate="}postData=password+"&"+email+"&"+endDate+"&"+firstName+"&"+lastName+"&"+entity+"&"+extra;"POST"==
a?(uri="users/",postData="userName="+encodeURIComponent(document.getElementById("userName").value)+"&"+postData):uri="users/"+encodeURIComponent(document.getElementById("userName").value);$.ajax({url:uri,dataType:"json",type:a,data:postData,success:startEditCurrentUser,error:displayErrorV2})}function startEditUser(a){currentUserUri=a;$.getJSON(a,editUser).error(displayErrorV2)}function startEditCurrentUser(){startEditUser(currentUserUri)}
...
function editUser(a){$.get("resources/templates/userEdit.php",function(b){userDate=new Date(a.endDate);dateFormated=userDate.format("<?php echo Localization::getJSString("date.format.parseexact")?>");null!=a.lastTokenLogin?(loginDate=new Date(a.lastTokenLogin),lastTokenLogin=loginDate.format("<?php echo Localization::getJSString("date.format.parseexact.long")?>")):lastTokenLogin="";currentUser=a;$("#content").html(b.replaceAll("{userNameAsLabel}",a.userName).replaceAll("{userNameInputType}",
"hidden").replaceAll("{userName}",a.userName).replaceAll("{password}",a.password).replaceAll("{firstName}",a.firstName).replaceAll("{lastName}",a.lastName).replaceAll("{entity}",a.entity).replaceAll("{emailAddress}",a.emailAddress).replaceAll("{endDate}",dateFormated).replaceAll("{lastTokenLogin}",lastTokenLogin).replaceAll("{extra}",null==a.extra?"":a.extra).replaceAll("{uri}",a.uri));$("#userEndDate").datepicker();setUserModified(!1)})}
...
table.removeChild(rowPattern);b=[];var c=[],e=[],f=[],g=[];for(i=0;i<a.length;i++){var h=new Date;h.setISO8601(a[i].endDate);addItem(b,a[i].userName);addItem(c,a[i].emailAddress,!0);addItem(e,a[i].entity,!0);addItem(f,a[i].firstName,!0);addItem(g,a[i].lastName,!0);newRow=rowPattern.cloneNode(!0);newRow.removeAttribute("id");newRow.removeAttribute("style");newRow.className=newRow.className+" tabular_table_body"+i%2;newRow.innerHTML=newRow.innerHTML.replaceAll("{userList[i].userName}",a[i].userName).replaceAll("{userList[i].emailAddress}",
a[i].emailAddress).replaceAll("{userList[i].uri}",a[i].uri).replaceAll("{userList[i].endDate}",h.format("<?php echo Localization::getJSString("date.format.parseexact")?>"));table.appendChild(newRow);edit=document.getElementById("btnEdit");del=document.getElementById("btnDelete");"Admin"===a[i].userName?del.remove():del.removeAttribute("id");edit.removeAttribute("id")}0==a.length&&$("#usersList").hide();$("#userNameFilter").autocomplete({source:b,minLength:0});$("#emailAddressFilter").autocomplete({source:c,
minLength:0});$("#entityFilter").autocomplete({source:e,minLength:0});$("#firstNameFilter").autocomplete({source:f,minLength:0});$("#lastNameFilter").autocomplete({source:g,minLength:0})})}function deleteUser(a,b){confirm("<?php echo Localization::getJSString("user.delete.confirm")?> "+b+"?")&&$.ajax({url:a,dataType:"json",type:"DELETE",success:showUsers,error:displayErrorV2})}
