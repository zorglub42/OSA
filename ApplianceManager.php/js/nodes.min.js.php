...
!1})}
function saveOrUpdateNode(a){if("80"==$("#port").val()&&document.getElementById("isHTTPS").checked&&!confirm("<?php echo Localization::getJSString("node.https-on-80-warning")?>")||"443"==$("#port").val()&&!document.getElementById("isHTTPS").checked&&!confirm("<?php echo Localization::getJSString("node.http-on-443-warning")?>"))return!1;showWait();var b=document.getElementById("PKfileuploadFLD"),c=document.getElementById("CERTfileuploadFLD"),d=document.getElementById("CHAINfileuploadFLD");del=
!0;removeSSLSetting&&0==b.files.length&&0==c.files.length&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/cert",dataType:"json",async:!1,type:"DELETE",success:function(a){del=!0},error:displayErrorV2}),del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/privateKey",dataType:"json",type:"DELETE",async:!1,success:function(a){del=!0},error:displayErrorV2})));removeCASSLSetting&&0==d.files.length&&del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/chain",dataType:"json",type:"DELETE",
async:!1,success:function(a){del=!0},error:displayErrorV2}));if(!del)return hideWait(),!1;nodeName="nodeName="+encodeURIComponent($("#nodeNameFld").val());localIP="localIP="+encodeURIComponent(document.getElementById("localIP").value);port="port="+encodeURIComponent(document.getElementById("port").value);nodeDescription="nodeDescription="+encodeURIComponent(document.getElementById("nodeDescription").value);additionalConfiguration="additionalConfiguration="+encodeURIComponent($("#additionalConfiguration").val());
serverFQDN="serverFQDN="+encodeURIComponent(document.getElementById("serverFQDN").value);isHTTPS=document.getElementById("isHTTPS").checked?"isHTTPS=1":"isHTTPS=0";postData=localIP+"&"+serverFQDN+"&"+port+"&"+nodeDescription+"&"+additionalConfiguration+"&"+isHTTPS;"PUT"==a?businessUrl="nodes/"+encodeURIComponent(document.getElementById("nodeNameFld").value):(businessUrl="nodes/",postData="nodeName="+encodeURIComponent(document.getElementById("nodeNameFld").value)+"&"+postData);if("POST"==a&&(postOk=
!1,$.ajax({url:businessUrl,dataType:"json",type:a,data:postData+"&apply=0",async:!1,success:function(){postOk=!0},error:displayErrorV2}),!postOk))return!1;ssl=error=!1;nodeURI="nodes/"+encodeURIComponent(document.getElementById("nodeNameFld").value);0<b.files.length&&($("#fileupload").fileupload(),$("#fileupload").fileupload("send",{async:!1,files:b.files,url:nodeURI+"/privateKey"}).error(function(a,b,c){error=error||!0;$("#fileupload").fileupload("destroy");displayErrorV2(a,b,c)}),ssl=!0,$("#fileupload").fileupload("destroy"));
0<c.files.length&&($("#fileupload").fileupload(),$("#fileupload").fileupload("send",{async:!1,files:c.files,url:nodeURI+"/cert"}).error(function(a,b,c){error=error||!0;$("#fileupload").fileupload("destroy");displayErrorV2(a,b,c)}),ssl=!0,$("#fileupload").fileupload("destroy"));0<d.files.length&&($("#fileupload").fileupload(),$("#fileupload").fileupload("send",{async:!1,files:d.files,url:nodeURI+"/chain"}).error(function(a,b,c){error=error||!0;$("#fileupload").fileupload("destroy");displayErrorV2(a,
b,c)}),ssl=!0,$("#fileupload").fileupload("destroy"));if(error)return!1;$.ajax({url:businessUrl,dataType:"json",type:"PUT",data:postData,success:showNodes,error:displayErrorV2})}function saveNewNode(){saveOrUpdateNode("POST")}
