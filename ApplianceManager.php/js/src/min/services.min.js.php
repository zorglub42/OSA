...
function populateNodeListFilter(a){defaultLabel="<?php echo Localization::getJSString("service.label.chooseNode")?>";$("#nodeNameFilter").append($("<option>",{value:"",text:defaultLabel}));$.each(a,function(a,b){$("#nodeNameFilter").append($("<option>",{value:b.nodeName,text:""!=b.nodeDescription?b.nodeDescription:b.nodeName}))});""!=nodeNameFilterPrevVal&&$("#nodeNameFilter option[value="+nodeNameFilterPrevVal+"]").prop("selected","selected")}
...
a[i].groupName).replaceAll("{serviceList[i].frontEndEndPoint}",a[i].frontEndEndPoint).replaceAll("{serviceList[i].backEndEndPoint}",a[i].backEndEndPoint).replaceAll("{serviceList[i].uri}",a[i].uri),table.appendChild(newRow),edit=document.getElementById("btnEdit"),del=document.getElementById("btnDelete"),a[i].serviceName.startsWith("ApplianceManagerAdmin")?del.remove():del.removeAttribute("id"),edit.removeAttribute("id");setServiceModified(!1);startLoadNodes()})}
function deleteService(a,c){confirm("<?php echo Localization::getJSString("service.delete.confirm")?> "+c+"?")&&(showWait(),$.ajax({url:a,dataType:"json",type:"DELETE",success:showServices,error:displayErrorV2}))}function resetServiceFilter(){$("#serviceNameFilter").val("");$("#serviceGroupNameFilter").val("");$("#frontEndEndPointFilter").val("");$("#backEndEndPointFilter").val("");$('#nodeNameFilter option[value=""]').prop("selected","selected");showServices()}
...
$.ajax({url:"./services/",dataType:"json",type:"GET",data:prms,success:displayServiceList,error:displayErrorV2})}$(function(){$("#listService").click(resetServiceFilter);$("#addService").click(addService)});
