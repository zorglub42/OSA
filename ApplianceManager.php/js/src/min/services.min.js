ZZOPEN();require_once="../include/Constants.php";require_once="../include/Settings.ini.php";require_once="../include/Localization.php";ZZCLOSE();var currentService,currentServiceGroup,serviceModified,nodesLoaded=!1,serviceNameFilterPrevVal="",serviceGroupNameFilterPrevVal="",frontEndEndPointFilterPrevVal="",backEndEndPointFilterPrevVal="",nodeNameFilterPrevVal="";
function setServiceModified(a){(serviceModified=a)?setActionButtonEnabled("saveService",!0):setActionButtonEnabled("saveService",!1);""!=$("#additionalConfiguration").val()?$("#warnAdditionalConfig").show():$("#warnAdditionalConfig").hide()}
...
function checkUserAuth(){document.getElementById("isUserAuthenticationEnabled").checked?($("#allowAnonymous").show(),$("#group").show(),$("#idForwarding").show(),$("#userQuota").show(),$("#loginForm").show(),document.getElementById("isAnonymousAllowed").checked?($("#isIdentityForwardingEnabled").prop("checked",!0),$("#isIdentityForwardingEnabled").prop("disabled",!0)):$("#isIdentityForwardingEnabled").prop("disabled",!1),cbForwardIdentClicked()):($("#loginForm").hide(),$("#allowAnonymous").hide(),
$("#group").hide(),$("#idForwarding").hide(),$("#idMapping").hide(),$("#userQuota").hide());setServiceModified(!0)}function cbForwardIdentClicked(){document.getElementById("isIdentityForwardingEnabled").checked?$("#idMapping").show():$("#idMapping").hide();setServiceModified(!0)}function cbHeaderClicked(a){$("#"+a+"Header").prop("disabled",!document.getElementById("cb"+a+"Header").checked);setServiceModified(!0)}
...
cbIsHitLoggingEnabled).replaceAll("{additionalConfiguration}",null==a.additionalConfiguration?"":a.additionalConfiguration));$(function(){$("#tabs").tabs()});document.getElementById("isGlobalQuotasEnabled").checked=0!=a.isGlobalQuotasEnabled;setQuotasVisibility();setNodesVisiblility();startPopulateGroups();checkUserAuth();$("#mainForm").height($("#tabs").height()+10);$("#mainForm").click(function(){$("#mainForm").height($("#tabs").height()+10)});cbForwardIdentClicked();$.getJSON(a.uri+"/headers-mapping",
function(a){for(i=0;i<a.length;i++)$("#cb"+a[i].userProperty+"Header").prop("checked",!0),$("#"+a[i].userProperty+"Header").val(a[i].headerName),cbHeaderClicked(a[i].userProperty),setServiceModified(!1)})})}
...
(postData+="&"+reqSec+"&"+reqDay+"&"+reqMonth);"PUT"==a?businessUrl="services/"+encodeURIComponent(document.getElementById("serviceName").value):(businessUrl="services/",postData="serviceName="+encodeURIComponent(document.getElementById("serviceName").value)+"&"+postData);document.getElementById("onAllNodes").checked||(postData+="&noApply=");showWait();$.ajax({url:businessUrl,dataType:"json",type:a,data:postData,success:setHeadersMappings,error:displayErrorV2})}
function setHeadersMappings(){var a=[];ZZOPEN();document.getElementById("cbZZOPEN() // echo userProperties[$i]ZZCLOSE() //Header").checked&&(m={},a[0]=m,a[0].userProperty="ZZOPEN() // echo userProperties[$i]ZZCLOSE() //",a[0].headerName=$("#ZZOPEN() // echo userProperties[$i]ZZCLOSE() //Header").val());ZZOPEN();$.ajax({url:"services/"+encodeURIComponent($("#serviceName").val())+"/headers-mapping",dataType:"json",type:"POST",contentType:"application/json",data:JSON.stringify(a),success:manageNodesList,
...
"").replaceAll("{cbIsUserAuthenticationEnabled}","").replaceAll("{loginFormUri}","").replaceAll("{cbIsAnonymousAllowed}","").replaceAll("{cbIdentFwd}","").replaceAll("{backEndUsername}","").replaceAll("{backEndPassword}","").replaceAll("{cbOnAllNodes}","checked").replaceAll("{cbIsHitLoggingEnabled}","").replaceAll("{additionalConfiguration}",""));$(function(){$("#tabs").tabs()});startPopulateGroups();setQuotasVisibility();setNodesVisiblility();$("#mainForm").height($("#tabs").height()+10);$("#mainForm").click(function(){$("#mainForm").height($("#tabs").height()+
10)});ZZOPEN();$("#cbZZOPEN() // echo $propertyZZCLOSE() //Header").prop("checked",!0);$("#ZZOPEN() // echo $propertyZZCLOSE() //Header").val("ZZOPEN() // echo defaultHeadersName[$property]ZZCLOSE() //");ZZOPEN();setServiceModified(!1)})}function handelServiceFilterFormKeypress(a){if(13==a.keyCode)return showServices(),!1}
