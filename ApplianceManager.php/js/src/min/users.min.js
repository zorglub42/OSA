...
function editUser(a){$.get("resources/templates/userEdit.php",function(b){userDate=new Date(a.endDate);dateFormated=userDate.format("ZZOPEN echo Localization::getJSString(QUOTEdate.format.parseexactQUOTE)ZZCLOSE");null!=a.lastTokenLogin?(loginDate=new Date(a.lastTokenLogin),lastTokenLogin=loginDate.format("ZZOPEN echo Localization::getJSString(QUOTEdate.format.parseexact.longQUOTE)ZZCLOSE")):lastTokenLogin="";currentUser=a;$("#content").html(b.replaceAll("{userNameAsLabel}",a.userName).replaceAll("{userNameInputType}",
...
a[i].emailAddress).replaceAll("{userList[i].uri}",a[i].uri).replaceAll("{userList[i].endDate}",h.format("ZZOPEN echo Localization::getJSString(QUOTEdate.format.parseexactQUOTE)ZZCLOSE"));table.appendChild(newRow);edit=document.getElementById("btnEdit");del=document.getElementById("btnDelete");"Admin"===a[i].userName?del.remove():del.removeAttribute("id");edit.removeAttribute("id")}0==a.length&&$("#usersList").hide();$("#userNameFilter").autocomplete({source:b,minLength:0});$("#emailAddressFilter").autocomplete({source:c,
