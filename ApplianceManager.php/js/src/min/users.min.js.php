...
table.removeChild(rowPattern);for(i=0;i<a.length;i++)b=new Date,b.setISO8601(a[i].endDate),newRow=rowPattern.cloneNode(!0),newRow.removeAttribute("id"),newRow.removeAttribute("style"),newRow.className=newRow.className+" tabular_table_body"+i%2,newRow.innerHTML=newRow.innerHTML.replaceAll("{userList[i].userName}",a[i].userName).replaceAll("{userList[i].emailAddress}",a[i].emailAddress).replaceAll("{userList[i].uri}",a[i].uri).replaceAll("{userList[i].endDate}",b.format("<?php echo Localization::getJSString("date.format.parseexact")?>")),
table.appendChild(newRow),edit=document.getElementById("btnEdit"),del=document.getElementById("btnDelete"),"Admin"===a[i].userName?del.remove():del.removeAttribute("id"),edit.removeAttribute("id");0==a.length&&$("#usersList").hide()})}function deleteUser(a,b){confirm("<?php echo Localization::getJSString("user.delete.confirm")?> "+b+"?")&&$.ajax({url:a,dataType:"json",type:"DELETE",success:showUsers,error:displayErrorV2})}
