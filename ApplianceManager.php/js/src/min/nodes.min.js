...
showWait();var b=document.getElementById("PKfileuploadFLD"),c=document.getElementById("CERTfileuploadFLD"),d=document.getElementById("CHAINfileuploadFLD");del=!0;removeSSLSetting&&0==b.files.length&&0==c.files.length&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/cert",dataType:"json",async:!1,type:"DELETE",success:function(a){del=!0},error:displayErrorV2}),del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/privateKey",dataType:"json",type:"DELETE",async:!1,success:function(a){del=!0},
error:displayErrorV2})));removeCASSLSetting&&0==d.files.length&&del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/chain",dataType:"json",type:"DELETE",async:!1,success:function(a){del=!0},error:displayErrorV2}));if(!del)return hideWait(),!1;nodeName="nodeName="+encodeURIComponent($("#nodeNameFld").val());localIP="localIP="+encodeURIComponent(document.getElementById("localIP").value);port="port="+encodeURIComponent(document.getElementById("port").value);nodeDescription="nodeDescription="+encodeURIComponent(document.getElementById("nodeDescription").value);
