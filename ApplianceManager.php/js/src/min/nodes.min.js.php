var currentNode,nodeModified,removeSSLSetting=!1,removeCASSLSetting=!1,nodeNameToolTip="Enter the node name here<br>Node name is node identifier and can not be changed later",localIPToolTip="Enter the IP address on witch the node is binded (* for any available)",portToolTip="Enter the port on witch the node listen",serverFQDNToolTip="Enter a fully qulified node name (like mynode.mydomain.com)",nodeDescriptionToolTip="Enter the node description",isHTTPSToolTip="Check this box if the node should use HTTPS instead of HTTP",
isBasicAuthEnabledToolTip="Check this box if the node should support basic authentication mode",isCookieAuthEnabledToolTip="Check this box if the node should support cookie based authentication mode",manageCAcertToolTip="Check this checkbox if you need to add CA cert and CA-Chain cert",editNodeToolTip="Edit this node",deleteNodeToolTip="Delete this node",addNodeTooTip="Add a new servie to the system",nodeNameFilterPrevVal="",nodeDescriptionFilterPrevVal="",localIPFilterPrevVal="",portFilterPrevVal=
"",serverFQDNFilterPrevVal="";function setNodeModified(a){(nodeModified=a)?setActionButtonEnabled("saveNode",!0):setActionButtonEnabled("saveNode",!1);$("#isHTTPS").is(":checked")?$("#tabs").tabs("enable",1):$("#tabs").tabs("disable",1);""!=$("#additionalConfiguration").val()?$("#warnAdditionalConfig").show():$("#warnAdditionalConfig").hide()}function startEditNode(a){$.getJSON(a,editNode).error(displayErrorV2)}function updateNode(a){saveOrUpdateNode("PUT")}
function startResetCASSL(){confirm("<?php echo Localization::getJSString("node.deleteCASSL.confirm")?>")&&(removeCASSLSetting=!0,setNodeModified(!0),clearFileInput("CAfileuploadFLD"),clearFileInput("CHAINfileuploadFLD"))}function startResetSSL(){confirm("<?php echo Localization::getJSString("node.deleteSSL.confirm")?>")&&(removeSSLSetting=!0,setNodeModified(!0),clearFileInput("PKfileuploadFLD"),clearFileInput("CERTfileuploadFLD"))}
function loadNodeServices(a){showWait();$.getJSON(a+"/services/",displayNodeServices).error(displayErrorV2)}
...
function saveOrUpdateNode(a){if(!$("#isCookieAuthEnabled").is(":checked")&&!$("#isBasicAuthEnabled").is(":checked"))return alert("At least one authentication methode (basic authentication or cookie authentication) should be enabled"),!1;showWait();var b=document.getElementById("PKfileuploadFLD"),d=document.getElementById("CERTfileuploadFLD"),e=document.getElementById("CAfileuploadFLD"),f=document.getElementById("CHAINfileuploadFLD");del=!0;removeSSLSetting&&0==b.files.length&&0==d.files.length&&(del=
!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/cert",dataType:"json",async:!1,type:"DELETE",success:function(a){del=!0},error:displayErrorV2}),del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/privateKey",dataType:"json",type:"DELETE",async:!1,success:function(a){del=!0},error:displayErrorV2})));removeCASSLSetting&&0==e.files.length&&del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/ca",dataType:"json",type:"DELETE",async:!1,success:function(a){del=!0},error:displayErrorV2}));removeCASSLSetting&&
0==f.files.length&&del&&(del=!1,$.ajax({url:"nodes/"+currentNode.nodeName+"/chain",dataType:"json",type:"DELETE",async:!1,success:function(a){del=!0},error:displayErrorV2}));if(!del)return hideWait(),!1;nodeName="nodeName="+encodeURIComponent($("#nodeNameFld").val());localIP="localIP="+encodeURIComponent(document.getElementById("localIP").value);port="port="+encodeURIComponent(document.getElementById("port").value);nodeDescription="nodeDescription="+encodeURIComponent(document.getElementById("nodeDescription").value);
...
{async:!1,files:d.files,url:nodeURI+"/cert"}).error(function(a,b,c){error=error||!0;$("#fileupload").fileupload("destroy");displayErrorV2(a,b,c)}),ssl=!0,$("#fileupload").fileupload("destroy"));0<e.files.length&&($("#fileupload").fileupload(),$("#fileupload").fileupload("send",{async:!1,files:e.files,url:nodeURI+"/ca"}).error(function(a,b,c){error=error||!0;$("#fileupload").fileupload("destroy");displayErrorV2(a,b,c)}),ssl=!0,$("#fileupload").fileupload("destroy"));0<f.files.length&&($("#fileupload").fileupload(),
$("#fileupload").fileupload("send",{async:!1,files:f.files,url:nodeURI+"/chain"}).error(function(a,b,c){error=error||!0;$("#fileupload").fileupload("destroy");displayErrorV2(a,b,c)}),ssl=!0,$("#fileupload").fileupload("destroy"));if(error)return!1;$.ajax({url:businessUrl,dataType:"json",type:"PUT",data:postData,success:showNodes,error:displayErrorV2})}function saveNewNode(){saveOrUpdateNode("POST")}
...
a[i].localIP).replaceAll("{nodeList[i].port}",a[i].port).replaceAll("{nodeList[i].nodeDescription}",a[i].nodeDescription).replaceAll("{nodeList[i].uri}",a[i].uri),table.appendChild(newRow),edit=document.getElementById("btnEdit"),del=document.getElementById("btnDelete"),del.removeAttribute("id"),edit.removeAttribute("id");0==a.length&&$("#nodesList").hide()})}
function deleteNode(a,b){confirm("<?php echo Localization::getJSString("node.delete.confirm")?> "+b+"?")&&(showWait(),$.ajax({url:a,dataType:"json",type:"DELETE",success:showNodes,error:displayErrorV2}))}function resetNodeFilter(){$("#nodeNameFilter").val("");$("#nodeDescriptionFilter").val("");$("#localIPFilter").val("");$("#portFilter").val("");$("#serverFQDNFilter").val("");showNodes()}
function showNodes(){prms="order=nodeName";prms=prms+"&nodeNameFilter="+encodeURIComponent(getFilterValue("nodeNameFilter"));prms=prms+"&nodeDescriptionFilter="+encodeURIComponent(getFilterValue("nodeDescriptionFilter"));prms=prms+"&localIPFilter="+encodeURIComponent(getFilterValue("localIPFilter"));prms=prms+"&portFilter="+encodeURIComponent(getFilterValue("portFilter"));prms=prms+"&serverFQDNFilter="+encodeURIComponent(getFilterValue("serverFQDNFilter"));$.ajax({url:"./nodes/",dataType:"json",type:"GET",
data:prms,success:displayNodeList,error:displayErrorV2})}$(function(){$("#listNode").click(resetNodeFilter);$("#addNode").click(addNode)});
