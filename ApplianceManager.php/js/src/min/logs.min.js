...
0==fetchingMore&&""!=nextFetch&&(fetchingMore=1,showWait(),$.ajax({url:nextFetch,dataType:"json",type:"GET",success:addFetch,error:displayError}))})})}function addFetch(a){hideWait();fetchingMore=0;nextFetch=a.next;previousFetch=a.previous;$("#logsCount").html(a.length+" hits found");generateLogsTableRows(a.logs)}function startPopulateAutoCompleteServices(){$.getJSON("services/?withLog=1&order=serviceName",populatePopulateAutoCompleteServices).error(displayError)}
function startPopulateAutoCompleteUsers(){$.getJSON("users/?withLog=1&order=userName",populatePopulateAutoCompleteUsers).error(displayError)}function populatePopulateAutoCompleteServices(a){var b=[],c=0;for(i=0;i<a.length;i++)1==a[i].isHitLoggingEnabled&&(b[c++]=a[i].serviceName);$("#serviceName").autocomplete({source:b,minLength:0})}function populatePopulateAutoCompleteUsers(a){var b=["None"];for(i=0;i<a.length;i++)b[i+1]=a[i].userName;$("#userName").autocomplete({source:b,minLength:0})}
