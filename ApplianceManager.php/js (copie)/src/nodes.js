...
function startResetCASSL(){
	if (confirm("ZZOPEN echo Localization::getJSString(QUOTEnode.deleteCASSL.confirmQUOTE)ZZCLOSE")){
...
function startResetSSL(){
	if (confirm("ZZOPEN echo Localization::getJSString(QUOTEnode.deleteSSL.confirmQUOTE)ZZCLOSE")){
...
function deleteNode(nodeURI, nodeName){
	...
	if (confirm("ZZOPEN echo Localization::getJSString(QUOTEnode.delete.confirmQUOTE)ZZCLOSE " + nodeName + "?")){
		showWait();
		$.ajax({
			  url: nodeURI,
			  dataType: 'json',
