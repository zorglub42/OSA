...
function deleteUserQuotas(quotaURI, serviceName) {
	if (confirm("ZZOPEN echo Localization::getJSString(QUOTEuser.deleteQuota.confirmQUOTE)ZZCLOSE " + serviceName + "?")) {
		$.ajax({
			url : quotaURI,
			dataType : 'json',
			type : 'DELETE',
...
function deleteUserGroup(groupURI, groupName, userURI) {
	if (confirm("ZZOPEN echo Localization::getJSString(QUOTEuser.deleteGroup.confirmQUOTE)ZZCLOSE " + groupName + "?")) {
		$.ajax({
...
function saveOrUpdateUser(method) {
	...
	try{
		d=Date.parseExact(document.getElementById("userEndDate").value,"ZZOPEN echo Localization::getJSString(QUOTEdate.formatQUOTE)ZZCLOSE");
		d.setHours(12);
		endDate = "endDate="
...
function editUser(user) {
	$.get( "resources/templates/userEdit.php", function( data ) {
		...
		dateFormated = userDate.format("ZZOPEN echo Localization::getJSString(QUOTEdate.formatQUOTE)ZZCLOSE");
		currentUser=user;
	...
	$.get("resources/templates/userList.php", function( data ) {
		...
		for (i=0;i<userList.length;i++){
			...
			newRow.innerHTML=newRow.innerHTML.replaceAll("{userList[i].userName}", userList[i].userName)
											 ...
											 .replaceAll("{userList[i].endDate}", d.format("ZZOPEN echo Localization::getJSString(QUOTEdate.formatQUOTE)ZZCLOSE"));
			table.appendChild(newRow);
			edit=document.getElementById("btnEdit");
			del=document.getElementById("btnDelete");
...
function deleteUser(userURI, userName) {
	...
	if (confirm("ZZOPEN echo Localization::getJSString(QUOTEuser.delete.confirmQUOTE)ZZCLOSE " + userName + "?")) {
		$.ajax({
			url : userURI,
			dataType : 'json',
			type : 'DELETE',
