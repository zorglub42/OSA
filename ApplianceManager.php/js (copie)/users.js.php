...
function saveOrUpdateUser(method) {
	...
	try{
		d=Date.parseExact(document.getElementById("userEndDate").value,"<?php echo Localization::getString("date.format")?>");
		d.setHours(12);
		endDate = "endDate="
...
function editUser(user) {
	$.get( "resources/templates/userEdit.php", function( data ) {
		...
		dateFormated = userDate.format("<?php echo Localization::getString("date.format")?>");
		currentUser=user;
	...
	$.get("resources/templates/userList.php", function( data ) {
		...
		for (i=0;i<userList.length;i++){
			...
			newRow.innerHTML=newRow.innerHTML.replaceAll("{userList[i].userName}", userList[i].userName)
											 ...
											 .replaceAll("{userList[i].endDate}", d.format("<?php echo Localization::getString("date.format")?>"));
			table.appendChild(newRow);
			edit=document.getElementById("btnEdit");
			del=document.getElementById("btnDelete");
